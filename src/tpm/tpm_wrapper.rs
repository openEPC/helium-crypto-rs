...
impl TpmWrapper {
    ...
    fn tpm(&self) -> &Mutex<*mut FAPI_CONTEXT> {
        return self.tpm_ctx.as_ref().unwrap();
    }

    fn with_tpm<F, R>(&self, f: F) -> R
        where
            F: FnOnce(*mut FAPI_CONTEXT) -> R,
    {
        let tpm_ctx = self.tpm().lock().unwrap();
        f(*tpm_ctx)
    }
    ...
    pub fn init(&mut self) -> Result {
        unsafe {
            ...
            self.tpm_ctx = Some(Mutex::new(tpm_ctx));
        }
    ...
    fn deinit(&mut self) {
        unsafe {
            let mut tpm_ctx = *self.tpm().lock().unwrap();
    ...
    pub fn public_key(&self, key_path: &String) -> Result<Vec<u8>> {
        unsafe {
            let tpm_ctx = self.tpm().lock().unwrap();
            ...
            if result != TSS2_RC_SUCCESS {
                Self::free_esys_resources(&mut esys_ctx, esys_key_handle)?;
    ...
    pub fn ecdh(&self, x: &[u8], y: &[u8], path: &String) -> Result<Vec<u8>> {
        unsafe {
            let tpm_ctx = self.tpm().lock().unwrap();
    ...
    pub fn sign(&self, path: &String, digest: Vec<u8>) -> Result<Vec<u8>> {
        unsafe {
            ...
            let result = self.with_tpm(|tpm_ctx| Fapi_Sign(tpm_ctx,
                                                      CString::new(path.as_bytes()).unwrap().as_ptr(),
                                                      null_mut(),
                                                      digest.as_ptr(),
