...
use p256::{ecdsa};
use tss_esapi::{
    TctiNameConf, Context, handles::KeyHandle
};
use tss_esapi::handles::TpmHandle;
use tss_esapi::interface_types::algorithm::HashingAlgorithm;
use tss_esapi::interface_types::resource_handles::Hierarchy;
use tss_esapi::interface_types::session_handles::AuthSession;
use tss_esapi::structures::{Digest, EccParameter, EccPoint, HashScheme, MaxBuffer, Public, Signature, SignatureScheme};
...
pub fn ecdh(x: &[u8], y: &[u8], key_handle: u32) -> Result<Vec<u8>> {
    ...
    let raw_point = EccPoint::new(
        EccParameter::try_from(x)?,
        EccParameter::try_from(y)?,
    );
    let point = context.execute_with_sessions((Some(AuthSession::Password), None, None), |ctx| {ctx.ecdh_z_gen(KeyHandle::from(key_handle), raw_point)})?;
...
pub fn sign(key_handle: u32, msg: &[u8]) -> Result<ecdsa::Signature>{
    ...
    let (digest, hashcheck_ticket) = context.hash(MaxBuffer::try_from(msg)?, HashingAlgorithm::Sha256, Hierarchy::Owner)?;
    let digest = Digest::try_from(digest).map_err(|_| Other("faild to convert digest".to_string()))?;
    ...
    let raw_signature =
        context.execute_with_sessions(
            ...
            |ctx| {ctx.sign(KeyHandle::from(key_handle), digest.clone(), signature_scheme, hashcheck_ticket)})?;
    ...
    let signature = match raw_signature {
        Signature::EcDsa(sig) => {
            ...
            ecdsa::Signature::from_scalars(array_r, array_s).
